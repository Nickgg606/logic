...
# ==================== 輸入賽日資訊 ====================
def input_race_day():
    global base_url, start_race, end_race
    
    input_root = tk.Tk()
    input_root.title("設定賽日資訊")
    input_root.geometry("500x400")
    
    tk.Label(input_root, text="日期 (YYYY-MM-DD):", font=("Arial", 12)).pack(pady=20)
    date_entry = tk.Entry(input_root, font=("Arial", 12), width=20)
    date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))
    date_entry.pack(pady=10)
    
    tk.Label(input_root, text="場地 (ST 或 HV):", font=("Arial", 12)).pack(pady=20)
    venue_entry = tk.Entry(input_root, font=("Arial", 12), width=10)
    venue_entry.insert(0, "HV")
    venue_entry.pack(pady=10)
    
    tk.Label(input_root, text="開始場次:", font=("Arial", 12)).pack(pady=20)
    start_entry = tk.Entry(input_root, font=("Arial", 12), width=10)
    start_entry.insert(0, "1")
    start_entry.pack(pady=10)
    
    tk.Label(input_root, text="結束場次:", font=("Arial", 12)).pack(pady=20)
    end_entry = tk.Entry(input_root, font=("Arial", 12), width=10)
    end_entry.insert(0, "9")
    end_entry.pack(pady=10)
    
    def save():
        global base_url, start_race, end_race
        date = date_entry.get().strip()
        venue = venue_entry.get().strip().upper()
        try:
            start_race = int(start_entry.get().strip())
            end_race = int(end_entry.get().strip())
        except:
            messagebox.showerror("錯誤", "場次必須是數字")
            return
        
        if not date or not venue or start_race > end_race:
            messagebox.showerror("錯誤", "請填寫完整且正確資訊")
            return
        
        base_url = f"https://bet.hkjc.com/ch/racing/wpq/{date}/{venue}"
        input_root.destroy()
    
    tk.Button(input_root, text="下一步", command=save, font=("Arial", 14, "bold"), bg="#4CAF50", fg="white", width=20).pack(pady=30)
    
    input_root.mainloop()

# ==================== 自動填入 Excel 5點賠率 ====================
def auto_fill_five_odds(race_no, horse_names):
    if race_no not in five_odds_from_excel:
        print(f"警告：場次 {race_no} 無 Excel 5點賠率資料，使用空 dict")
        return {}
    
    excel_odds = five_odds_from_excel[race_no]
    odds = {}
    for horse_no in horse_names:
        if horse_no in excel_odds:
            odds[horse_no] = excel_odds[horse_no]
        else:
            print(f"警告：場次 {race_no} 馬號 {horse_no} 無 5點賠率，已忽略")
    return odds
...
def web_view():
    ...
    <body>
        <h1>獨贏賠率監控</h1>
        <p>狀態: <span id="status" class="status">{{ status }}</span></p>
        <p>最後更新: <span id="last_update">{{ last_update }}</span></p>
    ...
    """
    return render_template_string(html, **global_data)
